...
# Configurations
# ------------------------------------------------------------------------------
out_folder = '/media/heraqi/data/heraqi/int-end-to-end-ad/auc.carla.dataset_00/train1'
kill_running_carla_servers = True  # False
clear_old_data = True  # clears existing out_folder data
...
sim_width = 1280
sim_height = 720
# In _collect_data_auc2/calra/agent/command_follower.py (if controlling_agent' here = 'CommandFollower'):
#   'stop4TL': False,  # Stop for traffic lights
# In _collect_data_auc2/dataset_configurations/auc_dataset.py:
...
def collect(client, args):
    ...
    # lateral_noiser = Noiser('Spike', frequency=25, intensity=4, min_noise_time_amount=0.5)  # Default lateral noise
    lateral_noiser = Noiser('Spike', frequency=35, intensity=4, min_noise_time_amount=0.5)  # Moderate lateral noise
    # lateral_noiser = Noiser('Spike', frequency=40, intensity=8, min_noise_time_amount=1.0  # High lateral noise
    ...
    try:
        ...
        while carla_game.is_running() and episode_number < maximun_episode:
            ...
            if image_count >= NUMBER_OF_FRAMES_CAR_FLIES and not args.not_record:
                ...
                debug = False
                if debug:
                    print("Throttle=%.2f" % round(control.throttle, 2) + "->%.2f" % round(control_noise_f.throttle, 2) +
                          "  Brake=%.2f" % round(control.brake, 2) + "->" + "%.2f" % round(control_noise_f.brake, 2) +
                          "  Steer=%.2f" % round(control.steer, 2) + "->" + "%.2f" % round(control_noise_f.steer, 2))
...
def main():
    ...
    # Run Carla server
    bashCommand = carla_path + 'CarlaUE4.sh /Game/Maps/' + town + \
    ...
    '''bashCommand = carla_path + "CarlaUE4.sh /Game/Maps/" + town + " -windowed -world-port=" + str(port) + \
                  " -benchmark -fps=" + str(game_fps)'''
