...
class Recording(object):
    ...
    def __init__(self
        ...
        self._start_from_exp_pose = start_from_exp_pose
    ...
    def write_summary_results(self, experiment, pose, rep,
        ...
        with open(os.path.join(self._path, 'summary.csv'), 'a+', newline='') as ofd:  # , newline='' due to in Windows blank line is written
    ...
    def _create_log_files(self):
        ...
        if not self._experiment_exist():
            ...
            with open(os.path.join(self._path, 'summary.csv'), 'w', newline='') as ofd:  # , newline='' due to in Windows blank line is written
                w = csv.DictWriter(ofd, self._dict_summary.keys())
                w.writeheader()
    ...
    def _continue_experiment(self, continue_experiment):
        ...
        if self._experiment_exist():
            ...
            if continue_experiment:
                # Get latest results folders
                if not os.path.exists(new_path):
                    raise ValueError("No results exist to continue the experiment on it!")
                filename, file_extension = os.path.splitext(new_path)
                i = 1
                new_f_name = "{}-{}{}".format(filename, i, file_extension)
                existing_file = filename
                while os.path.exists(new_f_name):
                    existing_file = new_f_name
                    i += 1
                    new_f_name = "{}-{}{}".format(filename, i, file_extension)
                   
                new_path = existing_file 
                self._path = existing_file
                line_on_file = self._get_last_position()  # TODO: seems usless line because the caller ignores that returned value, also wrong because self._path = existing_file should be before
    ...
    def get_pose_and_experiment(self, number_poses_task):
        ...
        if line_on_file == 0:
            # To start from a specific experiment and pose for debugging (debug.py: continue_experiment should be False)
            # some model failing examples: 4-9 4-2(our route planner make it succeed) 10-21
            return self._start_from_exp_pose[1]-1, self._start_from_exp_pose[0]-1  # pose then experiment
        else:
