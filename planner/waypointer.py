...
from planner.converter import Converter
from planner.city_track import CityTrack
from planner import bezier
import copy
...
class Waypointer(object):
    ...
    # way_key_points_predicted default was 7 (number of planning graph cells)
    # #TODO: it should be propotional to OGM size (to save waypoint computations far away outside OGM)
    def __init__(self, city_name, way_key_points_predicted=12):
        ...
        self.way_key_points_predicted = way_key_points_predicted
    ...
    def add_extra_points(self, node_target, target_ori, node_source):
        ...
        while len(self._route) < 10:  # ADD EXTRA POINTS AFTER
            ...
            self._route = np.vstack((self._route, np.array(direction)))
    ...
    def get_next_waypoints(self, route, source, source_ori, target, target_ori):
        '''
        ...
        # The distance to the closest intersection: to avoid computing a new route when inside the route
        distance_node = self._city_track.closest_curve_position(track_source)

        self._route = copy.deepcopy(route)  # calculated route from car to future
        ...
        if distance_node > 2 and self._previous_source != track_source:
            ...
            self.last_trajectory, self.last_map_points = self.generate_final_trajectory(self.points)
            # self.generate_final_trajectory([np.array(self._converter.convert_to_pixel(source))] + self.points)
            ...
            # Added this to re-compute route always #TODO: might need to fix this hack
            self.add_extra_points(track_target, target_ori, track_source)
            self.points = self.graph_to_waypoints(self._route[1:(1 + self.way_key_points_predicted)])
            self.last_trajectory, self.last_map_points = self.generate_final_trajectory(self.points)
            # self.generate_final_trajectory([np.array(self._converter.convert_to_pixel(source))] + self.points)
            ...
            # self.generate_final_trajectory([np.array(self._converter.convert_to_pixel(source))] + self.points)
                ...
                for point in self.last_map_points:
                    ...
                    if (cross_product > 0.0 and sldist(point, self._converter.convert_to_pixel(source)) < 50) or \
                            sldist(point, self._converter.convert_to_pixel(source)) < 15.0:
                        self.last_trajectory.remove(self._converter.convert_to_world(point))  # = [self.make_world_map(point)] + self.last_trajc
