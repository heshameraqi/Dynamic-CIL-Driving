...
'''
model_inputs_mode should be either "1cam" or "3cams" or "3cams-pgm"
'''
def load_imitation_learning_network(model_inputs_mode, input_image, input_data,
    ...
    if model_inputs_mode == "1cam":
        ...
        return load_network_3cams(input_image, input_data, dropout_vector=dropout_vector, branch_config=branch_config,
                           is_training=is_training)
    elif model_inputs_mode == "3cams-pgm":
        return load_network_3cams_3cams_pgm(input_image, input_data, dropout_vector=dropout_vector,
                                     branch_config=branch_config, is_training=is_training)
    else:
        assert "Error: model_inputs_mode is defined with unkown value, it should be either 1cam or 3cams or 3cams-pgm"
...
def load_network_3cams(input_image1, input_image2, input_image3, input_image4, input_data,
                       dropout_vector=[1.0] * 8 + [0.7] * 2 + [0.5] * 2 + [0.5] * 1 + [0.5, 1.] * 5,
                       branch_config=[["Steer", "Gas", "Brake"], ["Steer", "Gas", "Brake"],
...
def load_network_3cams_3cams_pgm(input_image1, input_image2, input_image3, input_image4, input_data,
                                 dropout_vector=[1.0] * 8 + [0.7] * 2 + [0.5] * 2 + [0.5] * 1 + [0.5, 1.] * 5,
                                 branch_config=[["Steer", "Gas", "Brake"], ["Steer", "Gas", "Brake"],
                                                ["Steer", "Gas", "Brake"], ["Steer", "Gas", "Brake"],
                                                ["Speed"]], is_training=True):
